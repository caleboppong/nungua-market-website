// React hooks used for state, side effects, and filtered lists.
import React, { useEffect, useMemo, useState } from "react";
import { supabase } from "./supabaseClient";
import {
  ShoppingCart, MapPin, Phone, Clock, Plus, Trash2, Pencil,
  LogOut, PackageCheck, UserPlus, BarChart3, Upload, Store, Utensils, ArrowLeft, Bike
} from "lucide-react";

// Grocery shop product categories.
const shopCategories = ["All", "Fresh Produce", "Grains & Flour", "Cooking Ingredients", "Oils & Sauces", "Drinks", "Frozen Food", "Spices"];
...
const restaurantOpeningHours = [
  ...
  ["Thursday", "12pm–10pm"], ["Friday", "12pm–11pm"], ["Saturday", "12pm–11pm"], ["Sunday", "12pm–11pm"]
];
...
// Delivery zones and prices for the restaurant department.
// The postcode prefix is checked against this list.
...
// Cleans and standardises customer postcodes before checking delivery availability.
...
// Checks whether delivery is available for a postcode and returns the delivery fee/message.
function getDeliveryInfo(postcode) {
  ...
  if (direct !== undefined) return { allowed: true, fee: direct, message: `Delivery available to ${prefix}. Fee £${direct.toFixed(2)}.` };
  const broad = Object.entries(allowedDeliveryZones).find(([zone]) => prefix.startsWith(zone));
  if (broad) return { allowed: true, fee: broad[1], message: `Delivery available to ${prefix}. Fee £${broad[1].toFixed(2)}.` };
  return { allowed: false, fee: 0, message: "Sorry, delivery is not currently available to this postcode. Please choose collection." };
}

// Reusable admin form for both grocery products and restaurant foods.
...
// Small + / - quantity control used in both baskets.
...
// Main application component. It controls department selection, shop, restaurant, admin, carts, and orders.
export default function App() {
  ...
  // Automatically changes the slideshow images every 4.5 seconds.
  useEffect(() => {
  ...
  // Load initial data from Supabase when the app first opens.
  useEffect(() => {
    loadProducts();
    loadProductImages();
    loadFoods();
  ...
  // When an admin/customer logs in, load protected admin data such as orders/messages.
  useEffect(() => {
    if (session) {
      loadOrders();
  ...
  // Load grocery products from Supabase.
  ...
  // Load restaurant foods from Supabase.
  ...
  // Login for admin/customer using Supabase Auth.
  ...
  // Chooses the best product image: uploaded image first, then image URL, then fallback image.
  ...
  // Chooses the best restaurant food image.
  ...
  // Filters grocery products by category and search text.
  ...
  // Filters restaurant foods by category and search text.
  ...
  const totalRevenue = orders.reduce((s, o) => s + Number(o.total_price || 0), 0) + restaurantOrders.reduce((s, o) => s + Number(o.total_price || 0), 0);
  const lowStock = products.filter((p) => Number(p.stock) <= 5).length + foods.filter((f) => Number(f.stock) <= 5).length;

  // Adds a grocery product to the basket or increases quantity if already added.
  ...
  // Adds a restaurant food item to the restaurant basket.
  ...
  function changeFoodQty(id, amount) {
    setRestaurantCart((items) => items.map((i) => i.id === id ? { ...i, quantity: Math.max(0, i.quantity + amount) } : i).filter((i) => i.quantity > 0));
  }

  // Reusable image upload function for product images and food images.
  async function uploadFiles(files, itemId, bucket = "product-images", table = "product_images", key = "product_id") {
    ...
    for (const file of files) {
      ...
      await supabase.from(table).insert({ [key]: itemId, image_url: data.publicUrl });
    }
    return uploadedUrls;
  }
  ...
  async function uploadProductFiles(files, productId) {
    ...
    await loadProducts(); await loadProductImages();
  }
  ...
  async function uploadFoodFiles(files, foodId) {
    ...
    await loadFoods(); await loadFoodImages();
  }

  // Sends grocery order details to WhatsApp using a pre-filled message.
  function sendOrderToWhatsApp() {
    ...
    const itemsText = cart.map((item) => `${item.name} x ${item.quantity} = £${(Number(item.price) * item.quantity).toFixed(2)}`).join("\n");
  ...
  // Saves a grocery order into Supabase and opens WhatsApp confirmation.
  ...
  // Starts the grocery Stripe checkout using the existing Supabase Edge Function.
  async function payWithStripe() {
    if (cart.length === 0) return setMessage("Please add products to the basket first.");
    if (!customer.name || !customer.phone || !customer.email) return setMessage("Please enter name, phone, and email before card payment.");
    try {
      const response = await fetch("https://quvjpenvmdkgaesqzslv.supabase.co/functions/v1/create-checkout-session", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: cart, customer })
      });
      const data = await response.json();
      if (!response.ok) return setMessage(data.error || "Stripe function failed.");
      if (data.url) window.location.href = data.url; else setMessage("Stripe did not return a checkout URL.");
    } catch (error) { setMessage(error.message); }
  }

  // Saves a restaurant order into Supabase. Stripe for restaurant is connected later in Step 9.
  async function placeRestaurantOrder() {
    if (!restaurantCustomer.name || !restaurantCustomer.phone || restaurantCart.length === 0) return setMessage("Please enter customer name, phone number, and add food.");
    if (restaurantCustomer.fulfilment_method === "delivery" && !deliveryInfo.allowed) return setMessage(deliveryInfo.message);
    const { data: order, error } = await supabase.from("restaurant_orders").insert({
      ...
      admin_note: "Restaurant order submitted. Separate Stripe payment will be connected at Step 9."
    }).select().single();
    if (error) return setMessage(error.message);
    const items = restaurantCart.map((item) => ({ order_id: order.id, food_id: item.id, quantity: item.quantity, price: item.price }));
  ...
  // Restaurant Stripe payment.
  // Currently uses the SAME Stripe checkout as the grocery shop.
  // You can later connect a different Stripe account for the restaurant.
  ...
  // Admin: add new grocery product.
  async function addProduct(e) {
    e.preventDefault();
    if (!newProduct.name || !newProduct.price) return setMessage("Product name and price are required.");
    const payload = { ...newProduct, price: Number(newProduct.price), stock: Number(newProduct.stock || 0), available: Boolean(newProduct.available) && Number(newProduct.stock || 0) > 0 };
    const { data, error } = await supabase.from("products").insert(payload).select().single();
    if (error) return setMessage(error.message);
    if (newFiles.length > 0) await uploadProductFiles(newFiles, data.id);
    setNewFiles([]); setNewProduct(blankProduct); setMessage("Product added successfully."); loadProducts();
  }

  // Admin: add new restaurant food item.
  async function addFood(e) {
    e.preventDefault();
    if (!newFood.name || !newFood.price) return setMessage("Food name and price are required.");
    const payload = { ...newFood, price: Number(newFood.price), stock: Number(newFood.stock || 0), available: Boolean(newFood.available) && Number(newFood.stock || 0) > 0 };
    ...
    setNewFoodFiles([]); setNewFood(blankFood); setMessage("Food added successfully."); loadFoods();
  }

  function startEdit(product) { setEditing(product.id); setEditProduct({ ...product }); setEditFiles([]); }
  function startFoodEdit(food) { setEditingFood(food.id); setEditFood({ ...food }); setEditFoodFiles([]); }

  async function saveEdit(e) {
    e.preventDefault();
    const payload = { name: editProduct.name, category: editProduct.category, price: Number(editProduct.price), image_url: editProduct.image_url, stock: Number(editProduct.stock), description: editProduct.description, available: Boolean(editProduct.available) && Number(editProduct.stock) > 0 };
    const { error } = await supabase.from("products").update(payload).eq("id", editProduct.id);
    if (error) return setMessage(error.message);
    if (editFiles.length > 0) await uploadProductFiles(editFiles, editProduct.id);
    setEditing(null); setEditFiles([]); setMessage("Product updated."); loadProducts();
  }

  async function saveFoodEdit(e) {
    e.preventDefault();
    const payload = { name: editFood.name, category: editFood.category, price: Number(editFood.price), image_url: editFood.image_url, stock: Number(editFood.stock), description: editFood.description, available: Boolean(editFood.available) && Number(editFood.stock) > 0 };
    ...
    setEditingFood(null); setEditFoodFiles([]); setMessage("Food updated."); loadFoods();
  }

  async function deleteProduct(id) {
    if (!confirm("Delete this product?")) return;
    const { error } = await supabase.from("products").delete().eq("id", id);
    if (error) setMessage(error.message); else { setMessage("Product deleted."); loadProducts(); loadProductImages(); }
  }

  async function deleteFood(id) {
    if (!confirm("Delete this food item?")) return;
    const { error } = await supabase.from("restaurant_foods").delete().eq("id", id);
    if (error) setMessage(error.message); else { setMessage("Food deleted."); loadFoods(); loadFoodImages(); }
  }
  ...
  async function updateRestaurantOrderStatus(id, status) {
    ...
    if (error) setMessage(error.message); else loadRestaurantOrders();
  }

  // First page: lets customers choose Grocery Shop or Restaurant.
        ...
        <header className="department-hero">
          <div className="department-overlay">
            <div className="wrap">
              ...
              <p className="department-subtitle">African groceries and fresh restaurant meals from one trusted local business.</p>
              <div className="department-grid">
                <button className="department-card" onClick={() => { setDepartment("shop"); setShopTab("shop"); }}>
                  <Store size={48} />
                  <h2>Grocery Shop</h2>
                  <p>Shop African groceries, fresh produce, pantry goods, and household essentials.</p>
                  <span>Enter Shop →</span>
                </button>
                <button className="department-card restaurant" onClick={() => { setDepartment("restaurant"); setRestaurantTab("menu"); }}>
                  <Utensils size={48} />
                  <h2>Restaurant</h2>
  ...
  if (!department) return LandingPage();

  // Header/navigation changes depending on whether the user is in Shop or Restaurant.
    ...
    return (
      <header className={isShop ? "header shop-header" : "header restaurant-header"}>
        <div className="wrap top">
          <div>
            <button className="btn btn-light" onClick={() => setDepartment(null)}>
            ...
            {isShop ? (
              <div className="nav">
                <button className={"btn " + (shopTab === "shop" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("shop")}>Shop</button>
                <button className={"btn " + (shopTab === "contact" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("contact")}>Contact</button>
                {session && <button className={"btn " + (shopTab === "admin" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("admin")}>Admin Products</button>}
                {session && <button className={"btn " + (shopTab === "orders" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("orders")}>Orders</button>}
                {session && <button className={"btn " + (shopTab === "messages" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("messages")}>Messages</button>}
                {session && <button className={"btn " + (shopTab === "analytics" ? "btn-gold" : "btn-light")} onClick={() => setShopTab("analytics")}>Analytics</button>}
              </div>
            ) : (
              <div className="nav">
                <button className={"btn " + (restaurantTab === "menu" ? "btn-gold" : "btn-light")} onClick={() => setRestaurantTab("menu")}>Menu</button>
                <button className={"btn " + (restaurantTab === "delivery" ? "btn-gold" : "btn-light")} onClick={() => setRestaurantTab("delivery")}>Delivery Info</button>
                <button className={"btn " + (restaurantTab === "contact" ? "btn-gold" : "btn-light")} onClick={() => setRestaurantTab("contact")}>Contact</button>
                {session && <button className={"btn " + (restaurantTab === "food-admin" ? "btn-gold" : "btn-light")} onClick={() => setRestaurantTab("food-admin")}>Food Admin</button>}
                {session && <button className={"btn " + (restaurantTab === "restaurant-orders" ? "btn-gold" : "btn-light")} onClick={() => setRestaurantTab("restaurant-orders")}>Restaurant Orders</button>}
              </div>
            )}
          </div>
  ...
  // Login/signup panels are shown only when no user is logged in.
  ...
  // Grocery basket and checkout panel.
  function ShopBasket() {
    return (
      <div className="panel">
        ...
        {cart.length === 0 ? <p className="muted">Your basket is empty.</p> : cart.map(item => (
          <div className="cart-item" key={item.id}>
            ...
            <div><b>{item.name}</b><p className="muted">£{Number(item.price).toFixed(2)}</p><QuantityControls item={item} onChange={changeQty}/></div>
          </div>
        ))}
  ...
  // Restaurant basket with collection/delivery and postcode check.
  function RestaurantBasket() {
    return (
      <div className="panel">
        <h2>Restaurant Basket</h2>
        {restaurantCart.length === 0 ? <p className="muted">Your food basket is empty.</p> : restaurantCart.map(item => (
          <div className="cart-item" key={item.id}>
            <img src={item.image_url} alt={item.name} />
            <div><b>{item.name}</b><p className="muted">£{Number(item.price).toFixed(2)}</p><QuantityControls item={item} onChange={changeFoodQty}/></div>
          </div>
        ))}
  ...
  // Hero slideshow used for both grocery and restaurant pages.
    ...
    return (
      <div className="panel" style={{ padding: 0, overflow: "hidden", background: "#020617", color: "white" }}>
        ...
        <div style={{ display: "flex", gap: "8px", justifyContent: "center", padding: "12px" }}>
          {slides.map((_, index) => <button key={index} onClick={() => restaurant ? setCurrentRestaurantSlide(index) : setCurrentSlide(index)} aria-label={`Show slide ${index + 1}`} style={{ width: slide === index ? "28px" : "10px", height: "10px", borderRadius: "20px", border: 0, background: slide === index ? "#facc15" : "#cbd5e1", cursor: "pointer" }} />)}
        </div>
      </div>
    );
  }
    ...
    return (
      <>
        ...
        <main className="layout">
          <section>
            ...
            {shopTab === "shop" && <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "14px", marginTop: "18px" }}>
                {homeCategories.map((item) => <button key={item.title} onClick={() => setCategory(item.title)} className="panel" style={{ textAlign: "left", cursor: "pointer", border: category === item.title ? "2px solid #facc15" : "1px solid #e2e8f0" }}><div style={{ fontSize: "32px" }}>{item.emoji}</div><h3 style={{ marginBottom: 4 }}>{item.title}</h3><p className="muted small">{item.note}</p></button>)}
              </div>
              <div className="panel" id="shop-products" style={{ marginTop: "18px" }}>
                ...
                <div className="cats">{shopCategories.map(c => <button key={c} className={"btn " + (category === c ? "btn-dark" : "btn-light")} onClick={() => setCategory(c)}>{c}</button>)}</div>
              </div>
              <div className="grid">{visibleProducts.map(product => <article className="card" key={product.id}><img src={mainImage(product)} alt={product.name}/><div className="thumbs">{imagesFor(product.id).map(img => <img key={img.id} src={img.image_url} alt={product.name}/>)}</div><div className="card-body"><div className="row"><div><p className="muted small">{product.category}</p><h3>{product.name}</h3></div><span className={"badge " + (product.available ? "ok" : "no")}>{product.available ? "Available" : "Out"}</span></div><p className="muted">{product.description}</p><div className="row"><div><div className="price">£{Number(product.price).toFixed(2)}</div><p className="muted small">Stock: {product.stock}</p></div><button className="btn btn-gold" disabled={!product.available} onClick={() => addToCart(product)}>Add</button></div></div></article>)}</div>
            </>}
          ...
          <aside>{LoginPanels()}{ShopBasket()}</aside>
        </main>
        {Footer()}
      </>
    );
    ...
    return (
      <>
        ...
        <main className="layout">
          <section>
            ...
            {restaurantTab === "menu" && <>
              ...
              <div className="panel" style={{ marginTop: "18px" }}>
                ...
                <div className="cats">{foodCategories.map(c => <button key={c} className={"btn " + (foodCategory === c ? "btn-dark" : "btn-light")} onClick={() => setFoodCategory(c)}>{c}</button>)}</div>
              </div>
              {foods.length === 0 && <div className="notice">No restaurant food has been added yet. Login as admin and open Food Admin to add meals.</div>}
              <div className="grid">{visibleFoods.map(food => <article className="card" key={food.id}><img src={mainFoodImage(food)} alt={food.name}/><div className="thumbs">{foodImagesFor(food.id).map(img => <img key={img.id} src={img.image_url} alt={food.name}/>)}</div><div className="card-body"><div className="row"><div><p className="muted small">{food.category}</p><h3>{food.name}</h3></div><span className={"badge " + (food.available ? "ok" : "no")}>{food.available ? "Available" : "Out"}</span></div><p className="muted">{food.description}</p><div className="row"><div><div className="price">£{Number(food.price).toFixed(2)}</div><p className="muted small">Stock: {food.stock}</p></div><button className="btn btn-gold" disabled={!food.available} onClick={() => addFoodToCart(food)}>Add</button></div></div></article>)}</div>
            </>}
          ...
          <aside>{LoginPanels()}{RestaurantBasket()}</aside>
        </main>
        {Footer()}
      </>
    );
  ...
  // Shared contact page used by both departments.
          ...
          <div style={{ padding: "20px" }}>
            ...
            <form onSubmit={sendContactMessage}>
              ...
              <textarea
                ...
                placeholder={
                  isRestaurant
                    ? "Ask about food orders, reservations, or delivery..."
                    : "Ask about groceries, stock, branch availability, or collection..."
                }
  ...
  // Shows delivery zones and fees for restaurant delivery.
  function DeliveryInfo() {
    return <div className="panel"><h2><Bike size={20}/> Delivery Area</h2><p>Restaurant delivery is currently available to these postcode zones:</p><div className="cats">{Object.entries(allowedDeliveryZones).map(([zone, fee]) => <span className="badge ok" key={zone}>{zone}: £{fee.toFixed(2)}</span>)}</div><p className="muted">Customers outside these zones should choose collection from the shop.</p></div>;
  }

  // Admin panel for managing grocery products.
  function ShopAdmin() {
    return <><div className="panel"><h2><Plus size={20}/> Add New Product</h2><ProductForm value={newProduct} setValue={setNewProduct} onSubmit={addProduct} submitText="Add Product" files={newFiles} setFiles={setNewFiles} categories={shopCategories}/></div><div className="grid">{products.map(product => <article className="card" key={product.id}><img src={mainImage(product)} alt={product.name}/><div className="card-body">{editing === product.id ? <ProductForm value={editProduct} setValue={setEditProduct} onSubmit={saveEdit} submitText="Save Changes" files={editFiles} setFiles={setEditFiles} categories={shopCategories}/> : <><div className="row"><h3>{product.name}</h3><span className={"badge " + (product.available ? "ok" : "no")}>{product.available ? "Available" : "Out"}</span></div><p className="muted">{product.category}</p><div className="price">£{Number(product.price).toFixed(2)}</div><p>Stock: {product.stock}</p><div className="row"><button className="btn btn-dark" onClick={() => startEdit(product)}><Pencil size={15}/> Edit</button><button className="btn btn-red" onClick={() => deleteProduct(product.id)}><Trash2 size={15}/> Delete</button></div></>}</div></article>)}</div></>;
  }

  // Admin panel for managing restaurant food items.
  function FoodAdmin() {
    return <><div className="panel"><h2><Plus size={20}/> Add New Food</h2><ProductForm value={newFood} setValue={setNewFood} onSubmit={addFood} submitText="Add Food" files={newFoodFiles} setFiles={setNewFoodFiles} categories={foodCategories}/></div><div className="grid">{foods.map(food => <article className="card" key={food.id}><img src={mainFoodImage(food)} alt={food.name}/><div className="card-body">{editingFood === food.id ? <ProductForm value={editFood} setValue={setEditFood} onSubmit={saveFoodEdit} submitText="Save Changes" files={editFoodFiles} setFiles={setEditFoodFiles} categories={foodCategories}/> : <><div className="row"><h3>{food.name}</h3><span className={"badge " + (food.available ? "ok" : "no")}>{food.available ? "Available" : "Out"}</span></div><p className="muted">{food.category}</p><div className="price">£{Number(food.price).toFixed(2)}</div><p>Stock: {food.stock}</p><div className="row"><button className="btn btn-dark" onClick={() => startFoodEdit(food)}><Pencil size={15}/> Edit</button><button className="btn btn-red" onClick={() => deleteFood(food.id)}><Trash2 size={15}/> Delete</button></div></>}</div></article>)}</div></>;
  }

  // Admin view for grocery orders.
  function ShopOrders() {
    return <div className="panel"><h2><PackageCheck size={20}/> Grocery Orders</h2><button className="btn btn-light" onClick={loadOrders}>Refresh Orders</button>{orders.length === 0 && <p className="muted">No orders yet.</p>}{orders.map(order => <div className="order-item" key={order.id}><div style={{width:"100%"}}><div className="row"><h3>{order.customer_name}</h3><span className={"badge " + (order.status === "completed" ? "done" : "new")}>{order.status}</span></div><p><b>Phone:</b> {order.customer_phone}</p><p><b>Email:</b> {order.customer_email || "None"}</p><p><b>Address/note:</b> {order.customer_address || "None"}</p><p><b>Total:</b> £{Number(order.total_price).toFixed(2)}</p><p className="muted small">{new Date(order.created_at).toLocaleString()}</p><ul>{(order.order_items || []).map(item => <li key={item.id}>{item.products?.name || "Product"} × {item.quantity} — £{Number(item.price).toFixed(2)}</li>)}</ul><div className="cats">{["new","preparing","ready","completed","cancelled"].map(s => <button key={s} className="btn btn-light" onClick={() => updateOrderStatus(order.id, s)}>{s}</button>)}</div></div></div>)}</div>;
  }

  // Admin view for restaurant orders.
  function RestaurantOrders() {
    return <div className="panel"><h2><PackageCheck size={20}/> Restaurant Orders</h2><button className="btn btn-light" onClick={loadRestaurantOrders}>Refresh Orders</button>{restaurantOrders.length === 0 && <p className="muted">No restaurant orders yet.</p>}{restaurantOrders.map(order => <div className="order-item" key={order.id}><div style={{width:"100%"}}><div className="row"><h3>{order.customer_name}</h3><span className={"badge " + (order.status === "completed" ? "done" : "new")}>{order.status}</span></div><p><b>Phone:</b> {order.customer_phone}</p><p><b>Email:</b> {order.customer_email || "None"}</p><p><b>Method:</b> {order.fulfilment_method}</p><p><b>Address:</b> {order.customer_address || "None"} {order.customer_postcode || ""}</p><p><b>Subtotal:</b> £{Number(order.subtotal_price || 0).toFixed(2)} | <b>Delivery:</b> £{Number(order.delivery_fee || 0).toFixed(2)} | <b>Total:</b> £{Number(order.total_price).toFixed(2)}</p><p className="muted small">{new Date(order.created_at).toLocaleString()}</p><p className="muted small">{order.admin_note}</p><ul>{(order.restaurant_order_items || []).map(item => <li key={item.id}>{item.restaurant_foods?.name || "Food"} × {item.quantity} — £{Number(item.price).toFixed(2)}</li>)}</ul><div className="cats">{["new","accepted","preparing","ready","completed","cancelled"].map(s => <button key={s} className="btn btn-light" onClick={() => updateRestaurantOrderStatus(order.id, s)}>{s}</button>)}</div></div></div>)}</div>;
  }

  function Messages() { return <div className="panel"><h2>Customer Messages</h2><button className="btn btn-light" onClick={loadContactMessages}>Refresh Messages</button>{contactMessages.length === 0 && <p className="muted">No messages yet.</p>}{contactMessages.map(msg => <div className="order-item" key={msg.id}><div><h3>{msg.name}</h3><p><b>Email:</b> {msg.email || "None"}</p><p><b>Phone:</b> {msg.phone || "None"}</p><p>{msg.message}</p><p className="muted small">{new Date(msg.created_at).toLocaleString()}</p></div></div>)}</div>; }
  function Analytics() { return <div className="panel"><h2><BarChart3 size={20}/> Admin Analytics</h2><div className="stats"><div className="stat"><p className="muted">Grocery Products</p><h2>{products.length}</h2></div><div className="stat"><p className="muted">Restaurant Foods</p><h2>{foods.length}</h2></div><div className="stat"><p className="muted">All Orders</p><h2>{orders.length + restaurantOrders.length}</h2></div><div className="stat"><p className="muted">Revenue recorded</p><h2>£{totalRevenue.toFixed(2)}</h2></div><div className="stat"><p className="muted">Low stock items</p><h2>{lowStock}</h2></div></div></div>; }
  function Footer() { return <footer className="footer">Nungua Market Platform · Grocery Shop & Restaurant</footer>; }
