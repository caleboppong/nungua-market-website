...
import {
  ...
  LogOut, PackageCheck, UserPlus, BarChart3, Upload
} from "lucide-react";
...
export default function App() {
  ...
  function sendOrderToWhatsApp() {
    ...
    const shopWhatsAppNumber = "447475655959";
    ...
    const text =
      `New order from Nungua Market website\n\n` +
  ...
  async function placeOrder() {
    ...
    setMessage("Order received and WhatsApp order message opened.");
      ...
      <header className="header">
        <div className="wrap top">
          ...
          <div>
            ...
            {session ? (
              <button className="btn btn-light" onClick={signOut}><LogOut size={15} /> Logout</button>
            ) : (
      ...
      <main className="layout">
          ...
          {tab === "shop" && (
            <>
              <div className="panel">
                ...
                <input
                  className="search"
                  placeholder="Search gari, plantain, fufu..."
                  value={search}
                  ...
                  <article className="card" key={product.id}>
                    ...
                    <div className="card-body">
                      ...
                      <div className="row">
                        ...
                        <button className="btn btn-gold" disabled={!product.available} onClick={() => addToCart(product)}>
                          Add
                        </button>
                      </div>
                    </div>
                ...
                <div className="order-item" key={order.id}>
                  <div style={{ width: "100%" }}>
                    <div className="row">
                      ...
                      <span className={"badge " + (order.status === "completed" ? "done" : "new")}>
                        {order.status}
                      </span>
                    </div>
                    ...
                    <div className="cats">
                      {["new", "preparing", "ready", "completed", "cancelled"].map(s => (
                        <button key={s} className="btn btn-light" onClick={() => updateOrderStatus(order.id, s)}>
                          {s}
                        </button>
                      ))}
                    </div>
        ...
        <aside>
          ...
          <div className="panel">
            ...
            <p className="muted small">Card payment uses Stripe checkout with the basket total.</p>
          </div>
        </aside>
      </main>

      <footer className="footer">
        Nungua Market African Food · 32 High St, London E17 7LD · 020 7000 4115
      </footer>
    </div>
  );
}